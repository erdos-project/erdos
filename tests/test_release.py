        ...
        if not self.send_batch(self.batch_number):
            ...
            if self.counter % self.batch_size == 0:
                ...
                watermark_msg = WatermarkMessage(Timestamp(
                    coordinates = [self.batch_number]))
                self.batch_number += 1
                self.get_output_stream(self.output_stream_name).send(
                    watermark_msg)
        else:
...
class SinkOperator(Op):
    ...
    def on_completion(self, msg):
        print("[{}] Received watermark for {}".format(self.name, msg.timestamp))
...
def main(argv):
    ...
    source_op_2 = graph.add(SourceOperator, name = "SecondOperator",
                            init_args = {
                                'batch_size': BATCH_SIZE,
    ...
    release_op = graph.add(EarlyReleaseOperator, name = "ReleaseOperator",
                            init_args = {
