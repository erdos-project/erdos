...
for PYBIN in /opt/python/{cp35-cp35m,cp36-cp36m,cp37-cp37m,cp38-cp38}/bin; do
    ...
    # Add the python directory to the path so the ERDOS build doesn't
    # error when it invokes python3 in build.rs.
    PATH="$PYBIN:$PATH" "${PYBIN}/python" python/setup.py bdist_wheel
done
