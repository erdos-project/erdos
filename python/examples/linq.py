...
class SinkOp(Sink):
    ...
    def on_data(self, context: SinkContext, data: Any):
        print(f"SinkOp ({context.config.name}): Received data: {data} for "
...
def main():
    ...
    source_stream = graph.connect_source(SendOp, OperatorConfig())
    ...
    graph.connect_sink(SinkOp, OperatorConfig(name="MergedOutput"),
                       merged_stream)
    graph.run()
