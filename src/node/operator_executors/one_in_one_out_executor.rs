...
use crate::{
    dataflow::{
        operator::{
            OneInOneOutSetupContext, OperatorConfig, ReadOnlyOneInOneOut,
            ReadOnlyOneInOneOutContext, WriteableOneInOneOut, WriteableOneInOneOutContext,
        },
        stream::WriteStreamT,
        Data, Message, ReadOnlyState, ReadStream, StreamT, Timestamp, WriteStream,
        WriteableState,
    },
    node::{
...
pub struct NewOneInOneOutExecutor<T>
where
...
impl<T> NewOneInOneOutExecutor<T>
where
    T: Data + for<'a> Deserialize<'a>,
{
    pub fn new(
...
impl<T> OperatorExecutorT for NewOneInOneOutExecutor<T>
where
    T: Data + for<'a> Deserialize<'a>,
{
    fn execute<'a>(
...
pub struct ReadOnlyOneInOneOutMessageProcessor<O, S, T, U, V, W>
where
...
impl<O, S, T, U, V, W> ReadOnlyOneInOneOutMessageProcessor<O, S, T, U, V, W>
where
    O: 'static + ReadOnlyOneInOneOut<S, T, U, V, W>,
    S: ReadOnlyState<V, W>,
    T: Data + for<'a> Deserialize<'a>,
...
impl<O, S, T, U, V, W> OneInMessageProcessorT<T>
    for ReadOnlyOneInOneOutMessageProcessor<O, S, T, U, V, W>
where
    O: 'static + ReadOnlyOneInOneOut<S, T, U, V, W>,
    S: ReadOnlyState<V, W>,
    T: Data + for<'a> Deserialize<'a>,
    ...
    W: 'static + Send + Sync,
{
    ...
    fn cleanup(&mut self) {
        if !self.write_stream.is_closed() {
            self.write_stream
                ...
                .expect(&format!(
                    "[ReadOnlyOneInOneOut] Error sending Top watermark for operator {}",
                    self.config.get_name()
                ));
        }
    }
    ...
    fn message_cb_event(&mut self, msg: Arc<Message<T>>) -> OperatorEvent {
        ...
        OperatorEvent::new(
            ...
            move || {
                operator.on_data(
                    &ReadOnlyOneInOneOutContext::new(time, config, &state, write_stream),
                    msg.data().unwrap(),
                )
            },
    ...
    fn watermark_cb_event(&mut self, timestamp: &Timestamp) -> OperatorEvent {
        ...
        if self.config.flow_watermarks {
            ...
            OperatorEvent::new(
                ...
                move || {
                    operator.on_watermark(&mut ReadOnlyOneInOneOutContext::new(
                        time,
                        config,
                        &state,
                        write_stream,
...
pub struct WriteableOneInOneOutMessageProcessor<O, S, T, U, V>
where
...
impl<O, S, T, U, V> WriteableOneInOneOutMessageProcessor<O, S, T, U, V>
where
    O: 'static + WriteableOneInOneOut<S, T, U, V>,
    S: WriteableState<V>,
    T: Data + for<'a> Deserialize<'a>,
...
impl<O, S, T, U, V> OneInMessageProcessorT<T>
    for WriteableOneInOneOutMessageProcessor<O, S, T, U, V>
where
    O: 'static + WriteableOneInOneOut<S, T, U, V>,
    S: WriteableState<V>,
    T: Data + for<'a> Deserialize<'a>,
    ...
    fn cleanup(&mut self) {
        if !self.write_stream.is_closed() {
            self.write_stream
                .send(Message::new_watermark(Timestamp::Top))
                .expect(&format!(
                    "[WriteableOneInOneOut] Error sending Top watermark for operator {}",
                    self.config.get_name()
                ));
        }
    }
        ...
        OperatorEvent::new(
            ...
            move || {
                operator.lock().unwrap().on_data(
                    &mut WriteableOneInOneOutContext::new(
                        time,
                        config,
            ...
            OperatorEvent::new(
                ...
                move || {
                    operator
                        .lock()
                        .unwrap()
                        .on_watermark(&mut WriteableOneInOneOutContext::new(
                            time,
                            config,
                            &mut state.lock().unwrap(),
                            write_stream,
