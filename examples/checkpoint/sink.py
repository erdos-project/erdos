...
class Sink(Op):
    ...
    def on_msg(self, msg):
        if msg.stream_name == 'watermark':
            ...
            if self.last_received_num is None:
                ...
                self._logger('received %d' % self._seq_num)
                self.last_received_num = self._seq_num
                # Build state
                ...
                self._logger('received DUPLICATE %d' % self._seq_num)
    ...
    def on_rollback_msg(self, msg):
        # Did not implement exact-once delivery so duplicate output is possible
