...
class GroundAgentOperator(Op):
    ...
    def stop_for_agents(self, wp_angle, wp_vector, vehicles, pedestrians,
        ...
        if FLAGS.stop_for_vehicles:
            for vehicle in vehicles:
                obs_vehicle_pos = (vehicle.position.location[0], vehicle.position.location[1])
                if agent_utils.is_vehicle_on_same_lane(self._vehicle_pos, obs_vehicle_pos, self._map):
                    new_speed_factor_v = agent_utils.stop_vehicle(
                        self._vehicle_pos, obs_vehicle_pos, wp_vector, speed_factor_v)
                    speed_factor_v = min(speed_factor_v, new_speed_factor_v)

        if FLAGS.stop_for_pedestrians:
            for pedestrian in pedestrians:
                ped_pos = (pedestrian.position.location[0], pedestrian.position.location[1])
                if agent_utils.is_pedestrian_hitable(ped_pos, self._map):
                    new_speed_factor_p = agent_utils.stop_pedestrian(
                        self._vehicle_pos, ped_pos, wp_vector, speed_factor_p)
        ...
        if FLAGS.stop_for_traffic_lights:
            for tl in traffic_lights:
                ...
                if (agent_utils.is_traffic_light_active(self._vehicle_pos, tl_pos, self._map) and
                    agent_utils.is_traffic_light_visible(self._vehicle_pos, tl_pos)):
                    new_speed_factor_tl = agent_utils.stop_traffic_light(
                        self._vehicle_pos, tl_pos, tl.state, wp_vector, wp_angle, speed_factor_tl)
